...
ipcMain.on('success:login', (event, msgUserId, msgFirstName, msgPosition, msgAccessLevel) => {
    ...
    switch (msgAccessLevel) {
        case 'admin':
            targetFile = 'index.html';
            break;
        case 'kasir':
            targetFile = 'kasir.html';
            break;
        case 'supervisor':
            targetFile = 'supervisor.html';
            break;
        default:
            targetFile = 'index.html';
            break;
    ...
    mainWindow.loadFile(path.join(__dirname, targetFile)).then(() => {
        // Mengirim data pengguna ke renderer process setelah halaman dimuat
        mainWindow.webContents.send('load:data', msgUserId, msgAccessLevel);
        mainWindow.webContents.send('unlock:app', { userId: msgUserId, firstName: msgFirstName, position: msgPosition, accessLevel: msgAccessLevel });
        loginModal.hide() 
    })
})
